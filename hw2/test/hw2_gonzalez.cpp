...
{
  switch (int rank = aCard.getRank()) {
  case 11: printf("%2s", "J"); break; //AG   2/5/12
  case 12: printf("%2s", "Q"); break; //changed so that the
  case 13: printf("%2s", "K"); break; //ranking for Ace is highest
  case 14: printf("%2s", "A"); break;
  default: printf("%2d", rank);
  }
  
  switch (suits suit = aCard.getSuit()) {
  case diamond: printf("%-18s"," of Diamonds"); break; //AG 2/5/12
  case spade: printf("%-18s"," of spades"); break;   //added some
  case heart: printf("%-18s"," of hearts"); break;   //better formatting
  case club: printf("%-18s"," of clubs"); break;
  }
  return out;
} // operator<<
